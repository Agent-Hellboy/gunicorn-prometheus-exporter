...
def test_worker_requests_metric():
    ...
    assert WORKER_REQUESTS._name == "gunicorn_worker_requests"
    assert (
        WORKER_REQUESTS._documentation
        == "Total number of requests handled by this worker"
    )
    assert WORKER_REQUESTS._labelnames == ("worker_id",)


def test_worker_request_duration_metric():
    """Test WORKER_REQUEST_DURATION metric configuration."""
    assert WORKER_REQUEST_DURATION._name == "gunicorn_worker_request_duration_seconds"
    assert WORKER_REQUEST_DURATION._documentation == "Request duration in seconds"
    assert WORKER_REQUEST_DURATION._labelnames == ("worker_id",)


def test_worker_memory_metric():
    """Test WORKER_MEMORY metric configuration."""
    assert WORKER_MEMORY._name == "gunicorn_worker_memory_bytes"
    assert WORKER_MEMORY._documentation == "Memory usage of the worker process"
    assert WORKER_MEMORY._labelnames == ("worker_id",)


def test_worker_cpu_metric():
    """Test WORKER_CPU metric configuration."""
    assert WORKER_CPU._name == "gunicorn_worker_cpu_percent"
    assert WORKER_CPU._documentation == "CPU usage of the worker process"
    assert WORKER_CPU._labelnames == ("worker_id",)


def test_worker_uptime_metric():
    """Test WORKER_UPTIME metric configuration."""
    assert WORKER_UPTIME._name == "gunicorn_worker_uptime_seconds"
    assert WORKER_UPTIME._documentation == "Uptime of the worker process"
    assert WORKER_UPTIME._labelnames == ("worker_id",)


def test_worker_failed_requests_metric():
    """Test WORKER_FAILED_REQUESTS metric configuration."""
    assert WORKER_FAILED_REQUESTS._name == "gunicorn_worker_failed_requests"
    assert (
        WORKER_FAILED_REQUESTS._documentation
        == "Total number of failed requests handled by this worker"
    )
    assert WORKER_FAILED_REQUESTS._labelnames == ("worker_id",)
...
def test_metric_registration():
    ...
    assert all(metric in collectors for metric in metric_names)
