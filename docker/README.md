...
docker run -d \
  ...
  gunicorn-prometheus-exporter:0.2.1
```
...
docker build -f docker/Dockerfile.app -t gunicorn-app:0.2.1 .
...
docker run -d \
  ...
  -p 9091:9091 \
  -v /tmp/prometheus_multiproc:/tmp/prometheus_multiproc \
  gunicorn-prometheus-exporter:0.2.1 sidecar
```
...
docker run -d \
  ...
  -p 9091:9091 \
  -v /tmp/prometheus_multiproc:/tmp/prometheus_multiproc \
  gunicorn-prometheus-exporter:0.2.1 standalone
```
...
```bash
docker run --rm \
  gunicorn-prometheus-exporter:0.2.1 health
```
