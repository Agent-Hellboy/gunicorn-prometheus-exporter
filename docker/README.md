...
docker run -d \
  ...
  gunicorn-prometheus-exporter:0.1.9
```
...
docker build -f docker/Dockerfile.app -t gunicorn-app:0.1.9 .
...
docker run -d \
  ...
  -p 9091:9091 \
  -v /tmp/prometheus_multiproc:/tmp/prometheus_multiproc \
  gunicorn-prometheus-exporter:0.1.9 sidecar
```
...
docker run -d \
  ...
  -p 9091:9091 \
  -v /tmp/prometheus_multiproc:/tmp/prometheus_multiproc \
  gunicorn-prometheus-exporter:0.1.9 standalone
```
...
```bash
docker run --rm \
  gunicorn-prometheus-exporter:0.1.9 health
```
...
docker tag gunicorn-prometheus-exporter:latest princekrroshan01/gunicorn-prometheus-exporter:0.1.9
```
...
docker push princekrroshan01/gunicorn-prometheus-exporter:0.1.9
```
