...
**Production Note**: Always use specific version tags (e.g., `0.2.0`) instead of `:latest` for reproducibility and stability. If the published image tag is not yet available, build the images locally with the commands above before deploying.
...
- `princekrroshan01/gunicorn-prometheus-exporter:0.2.0`
- `princekrroshan01/gunicorn-app:0.2.0`
