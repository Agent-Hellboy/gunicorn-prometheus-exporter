...
docker pull princekrroshan01/gunicorn-app:0.1.9

# Or build locally if the release has not been published yet:
# docker build -t princekrroshan01/gunicorn-prometheus-exporter:0.1.9 .
# docker build -f docker/Dockerfile.app -t princekrroshan01/gunicorn-app:0.1.9 .
```

**Production Note**: Always use specific version tags (e.g., `0.1.9`) instead of `:latest` for reproducibility and stability. If the published image tag is not yet available, build the images locally with the commands above before deploying.
...
- `princekrroshan01/gunicorn-prometheus-exporter:0.1.9`
- `princekrroshan01/gunicorn-app:0.1.9`
