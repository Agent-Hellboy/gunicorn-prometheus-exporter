"""
Example Gunicorn configuration for eventlet workers with Prometheus metrics.

This configuration uses the PrometheusEventletWorker which provides Prometheus
metrics support for Gunicorn's eventlet worker model.

Eventlet workers use greenlets for cooperative multitasking and are good
for I/O-bound applications with many concurrent connections.

Usage:
    gunicorn -c gunicorn_eventlet_async.conf.py async_app:app
"""
...
os.environ.setdefault("PROMETHEUS_METRICS_PORT", "9095")
