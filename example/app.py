...
def leak():
    ...
    leak_data = array('B', [random.randint(0, 255) for _ in range(leak_bytes)])
    app.memory_leak.append(leak_data)
    
    current_leak_mb = len(app.memory_leak) * leak_size_mb
    
    return f"Memory leak created: {leak_size_mb}MB added, total leak: {current_leak_mb}MB"
...
def crash():
    ...
    if force_crash or random.random() < crash_probability:
